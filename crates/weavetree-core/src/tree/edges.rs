...
use crate::tree::{ids::{ActionId, NodeId, StateKey}, outcomes::OutcomeSet, stats::EdgeStats};
...
impl ActionEdge {
    ...
    pub fn new(action: ActionId) -> Self {
        ActionEdge { edge_stats: EdgeStats::new(), outcomes: OutcomeSet::new(), action }
    }
    ...
    pub fn ucb_score(&self, n_parent: u64, c: f64) -> f64 {
        ...
        if self.edge_stats.is_unvisited() {
            ...
            self.edge_stats.q() + c * f64::sqrt(f64::ln(n_parent as f64) / self.edge_stats.visits() as f64)
        }
    }
